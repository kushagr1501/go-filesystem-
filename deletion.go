...
// FindInRecycleBin checks if a file with the given name exists in the Recycle Bin
func FindInRecycleBin(fileName string) (bool, error) {
	// Remove extension for matching
	nameWithoutExt := strings.TrimSuffix(fileName, filepath.Ext(fileName))
	escapedName := strings.ReplaceAll(nameWithoutExt, "'", "''")
	// If filename was "John's File", it becomes "John''s File"
	// This prevents PowerShell breaking on quotes
	script := fmt.Sprintf(`
        $shell = New-Object -ComObject Shell.Application
        $recycleBin = $shell.NameSpace(10)
        $item = $recycleBin.Items() | Where-Object { $_.Name -eq '%s' } | Select-Object -First 1
        if ($item) { 
            Write-Output "FOUND"
        } else { 
...
func RestoreFromRecycleBin(originalPath string) error {
	...
	nameWithoutExt := strings.TrimSuffix(fileName, filepath.Ext(fileName))
	escapedName := strings.ReplaceAll(nameWithoutExt, "'", "''")
	escapedDest := strings.ReplaceAll(originalPath, "'", "''")
	...
	script := fmt.Sprintf(`
        ...
        $item = $recycleBin.Items() | Where-Object { $_.Name -eq '%s' } | Select-Object -First 1
        if ($item) {
            Copy-Item -LiteralPath $item.Path -Destination '%s' -Force
        } else {
            exit 1
        }
    `, escapedName, escapedDest)
...
func UndoLastDeletion(history *DeletionHistory) error {
	...
	found, err := FindInRecycleBin(lastRecord.FileName)
	if err != nil {
		return fmt.Errorf("failed to search recycle bin: %v", err)
	}

	if !found {
		return fmt.Errorf("file '%s' not found in Recycle Bin - maybe already restored or permanently deleted?", lastRecord.FileName)
	}

	// Restore it
	if err := RestoreFromRecycleBin(lastRecord.OrigionalFilePath); err != nil {
		return err
	}
	...
	PrintSuccess(fmt.Sprintf("Restored: %s", lastRecord.FileName))
	PrintInfo(fmt.Sprintf("Location: %s", lastRecord.OrigionalFilePath))
